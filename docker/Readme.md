...
```commandline
cd [your-test-folder]
docker run -v $(pwd):/workspace iai-sdk-python-runner:x.y.z simaticai run_pipeline SegmentationPipeline-edge_1.zip --data images/test_set --test /workspace/results
```
...
```commandline
cd [your-test-folder]
docker run -v $(pwd):/workspace iai-sdk-python-runner:x.y.z simaticai run_pipeline SegmentationPipeline-edge_1.zip --config ./config.json
```
